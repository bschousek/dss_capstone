...
if (!file.exists(file.path(basedir,destpath,'en_US.blogs.bulktrain.txt'))) {
  infile=readLines(file(file.path(basedir,srcpath,'en_US/en_US.blogs.txt')),encoding = 'UTF-8')
  set.seed(42)
  training_index=as.logical(rbinom(length(infile),1,0.75))
  writeLines(infile[training_index],file(file.path(basedir,destpath,"en_US.blogs.bulktrain.txt")))
  test_index=!training_index
  writeLines(infile[test_index],file(file.path(basedir,destpath,"en_US.blogs.bulktest.txt")))
  ...
  writeLines(train1,file(file.path(basedir,destpath,"en_US.blogs.train1.txt")))
